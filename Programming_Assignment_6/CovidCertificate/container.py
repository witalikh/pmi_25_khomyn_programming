...
class Container:
    ...
    def __len__(self):
        """
        Get size of iterable via len()
        :return: number of records in container
        """
        return len(self._records)

    def __contains__(self, item: CovidCertificate):
        """
        (in) Checks if some certificate belongs to container
        :param item: CovidCertificate object
        :return: True/False
        """
    ...
    def add_record(self, certificate: CovidCertificate):
        ...
        """
        if isinstance(certificate, CovidCertificate):
            self._records.append(certificate)
        else:
            raise TypeError("expected only CovidCertificate")
    ...
    def edit_record(self, identifier, key, value):
        ...
        """
        assert key in CovidCertificate.keys()
